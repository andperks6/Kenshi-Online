...
void Core::OnLoadingGapDetected() {
    ...
    // or a pre-load network connection. NOT from Startup; engine
    // initialization gaps are not save game loads.
    if (current == ClientPhase::MainMenu || current == ClientPhase::GameReady ||
        ((current == ClientPhase::Connected || current == ClientPhase::Connecting) && !m_gameLoaded.load())) {
